...
def load_resources():
    ...
    try:
        ...
        print(f"Failed to load model or scaler: {str(e)}")
...
async def predict_batch(input_data: BatchPredictionInput):
    """Predict power output for multiple sets of features."""
    try:
        batch_input = np.array([prepare_input_data(item)[0] for item in input_data.data])
        batch_input_scaled = scaler.transform(batch_input)
        predictions = model.predict(batch_input_scaled)
        prediction_list = [float(pred[0]) for pred in predictions]
        return {"predictions": prediction_list}
...
async def predict_from_csv(input_path: str, output_path: Optional[str] = None):
    ...
    try:
        ...
        return {
            "file_path": output_path,
            "success": True,
            "message": f"Processed {len(predictions)} rows and saved to {output_path}"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"CSV processing error: {str(e)}")
...
async def reload_model():
    ...
    try:
        load_resources()
        return {"message": "Model and scaler successfully reloaded"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Model reload error: {str(e)}")

# Background Task: Auto-reload when model changes
def monitor_model_changes():
    """Monitor model file changes and reload when updated."""
    last_modified_time = os.path.getmtime(MODEL_PATH)
    while True:
        time.sleep(10)  # Check every 10 seconds
        current_modified_time = os.path.getmtime(MODEL_PATH)
        if current_modified_time != last_modified_time:
            print("Detected model change. Reloading...")
            load_resources()
...
async def startup_event():
    ...
    import threading
    threading.Thread(target=monitor_model_changes, daemon=True).start()
